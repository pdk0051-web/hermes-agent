...
def call_llm(
    ...
    *,
    ...
    try:
        ...
        #
        # NB (compression / #LEO): this same-provider retry stays on the SAME
        # OAuth Codex backend and never switches provider, so it does NOT
        # violate the compression OAuth-only guard below (``should_fallback``
        # is still forced False for compression — no API-key provider is ever
        # consulted).  The compressor's own escalated-timeout retry + operator
        # notification (context_compressor._generate_summary) layers on top.
        try:
            return _validate_llm_response(
...
async def async_call_llm(
    ...
    *,
    ...
    try:
        ...
        # for the rationale (incl. the compression / #LEO note: same-provider
        # retry never switches to an API-key provider). (PR #16587)
        try:
            return _validate_llm_response(
