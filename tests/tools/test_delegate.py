...
class TestDelegateHeartbeat(unittest.TestCase):
    ...
    def test_heartbeat_does_not_trip_idle_stale_while_inside_tool(self):
        ...
        # old _HEARTBEAT_STALE_CYCLES_IDLE=5 behavior would trip after
        # 0.25s and stop firing. We should see heartbeats continuing
        # through the full 1.6s run.
        with patch("tools.delegate_tool._HEARTBEAT_INTERVAL", 0.05), \
        ...
        # we should see substantially more heartbeats over 1.6s.
        self.assertGreater(
            len(touch_calls), 6,
            f"Heartbeat stopped too early while child was inside a tool; "
            f"got {len(touch_calls)} touches over 1.6s at 0.05s interval",
        )
