...
class TestTransientTransportRetry:
    ...
    def test_second_transient_failure_escalates_to_fallback(self):
        ...
        Uses a NON-compression task (web_extract): compression is OAuth-only
        (#LEO — agent/auxiliary_client.py forces ``should_fallback=False`` for
        ``task == "compression"``), so it deliberately never escalates to a
        cross-provider fallback.  Every other auxiliary task keeps the
        escalate-on-second-transient-failure behaviour this test pins.  The
        compression-specific guarantee is covered by
        tests/agent/test_compression_oauth_only_fallback.py.
        """
        primary = MagicMock()
        ...
        with (
            ...
            result = call_llm(task="web_extract", messages=[{"role": "user", "content": "hi"}])
